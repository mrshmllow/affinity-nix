...
rec {
  check =
    let
      revisionPath = "${affinityPathV3}/.revision";
      revision = "1";
      verbs = [
        "dotnet48"
        "corefonts"
  ...
  createGraphicalCheck = writeShellScriptBin "affinity-v3-gui-check" ''
  ...
  createInstaller =
    let
      source = sources.v3;
      check = createGraphicalCheck;
    in
    writeShellScriptBin "install-Affinity-v3" ''
      set -x
      ${stdShellArgs}
      ...
      function ensure_exists {
          ...
          (
              ...
              zenity --progress \
                --auto-close \
                --title="Affinity v3" \
                --text="Downloading the installer for Affinity v3.\n\nThis might take a moment.\n" 2>/dev/null
          )
      ...
      zenity --info \
          --title="Affinity v3" \
          --text="You will be prompted to install Affinity v3.\n\nPlease do not change the installation path."
  ...
  createPackage =
    let
      ...
      icon-package = icons.mkIconPackageFor;
    in
    pkgs.symlinkJoin {
      name = "Affinity v3";
      pname = "affinity-v3";
