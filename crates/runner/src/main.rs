...
use nix::{libc, mount::{MsFlags, mount, umount}, sched::{CloneFlags, unshare}, sys::wait::{WaitStatus, waitpid}, unistd::{ForkResult, fork}};
...
fn mount_privileged(paths: &Paths) {
    let options = format!("lowerdir={},upperdir={},workdir={}", paths.lower.display(), paths.upper.display(), paths.work.display());

    info!(mount_options = ?options);

    match mount(Some("overlay"), &paths.wine_prefix, Some("overlay"), MsFlags::empty(), Some(options.as_str())) {
        Ok(_) => {
            // launch

            let entries = fs::read_dir(&paths.wine_prefix).unwrap()
    ...
    let _ = umount(&paths.wine_prefix).inspect_err(|err| error!(error = ?err, "failed to unmount wine_prefix"));
    let _ = fs::remove_dir(&paths.wine_prefix).inspect_err(|err| error!(error = ?err, "failed to remove wine_prefix"));
}
...
fn main() {
    ...
    let base = BaseDir::new(if args.v3 {
        "affinity-v3"
    } else {
        "affinity"
    ...
    let paths = Paths {
        ...
        upper: base.data,
        work: base.state,
        wine_prefix: base.runtime.unwrap_or_else(|| std::env::temp_dir()).join(format!("affinity-nix-prefix-{}", std::process::id())),
    };

    paths.ensure_created().expect("failed to create temp directories");
    ...
    let unshare = unshare(CloneFlags::CLONE_NEWUSER | CloneFlags::CLONE_NEWNS | CloneFlags::CLONE_NEWPID);
